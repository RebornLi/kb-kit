...
from kb_common import ROOT_DEFAULT, EXCLUDE, parse_frontmatter, tokenize, CJK, iter_notes, load_note
...
def evaluate(rel, path, fm, text, body, vsim, target):
    ...
    score += int(25 * present / len(REQUIRED))
    if present < len(REQUIRED) - 2:
        reasons.append(f"缺{len(REQUIRED) - present} 必填字段")
    # 密度分 0-25
    d = density(text)
    score += 25 if d >= 0.15 else 18 if d >= 0.10 else 10 if d >= 0.05 else 3
    if d < 0.15:
    ...
    if vsim is None:
        ...
        score += 20
    # 价值信号 0-25
    hits = sum(1 for w in VALUE_KW if w in text)
    score += min(25, 5 + 3 * hits)
    # 路由判定
    tgt = fm.get("kb_target", "")
    route_to = tgt if _is_bucket(tgt) else ""
    action = "route" if route_to else "keep"
    if vsim and vsim >= MERGE_SIM_THRESHOLD:
        action = "merge"
    elif score < TRIAGE_THRESHOLDS[1][0]:
        action = TRIAGE_THRESHOLDS[2][1]  # trash
    elif score < TRIAGE_THRESHOLDS[0][0]:
        action = TRIAGE_THRESHOLDS[1][1]  # refine
    elif not route_to:
        action = "keep"
...
def triage(root, threshold):
    ...
    for p in sorted(Path(inbox_dir).glob("*.md")) if os.path.isdir(inbox_dir) else []:
        ...
        if best_v is not None and best_v >= threshold:
            ...
            res = evaluate(rel, p, fm, text, body, round(best_v, 3), best_rel)
        else:
            res = evaluate(rel, p, fm, text, body, None, "")
        results.append(res)
    return results
...
def apply_moves(root, results, move, trash):
    import subprocess
...
def triage_by_content_type(root, rel, fm, body, content_type):
    """按内容类型决定路由目标。

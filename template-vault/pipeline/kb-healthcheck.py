...
仅检测 + 报告，不自动修改。
"""
import os, re, sys, json, io, contextlib, html
    ...
    for fp in collect_md(root):
        ...
        for m in re.finditer(r"\[\[([^]]+)\]\]", content):
            ...
            if any(w in raw for w in NOISE_LINKS):
                continue
            if "," in raw or re.search(r"\d{4}-\d{2}-\d{2}", raw):   # 逗号分隔/日期引用，非真实链接
                continue
            path = link_path(m)
            if not path:
                continue
...
def run_checks(root, names):
    """执行指定 check（"all" 或列表），返回 [(name, count, detail_text), ...]。
    每个 check 的 stdout 已捕获进 detail_text（文本模式复用之），计数供 JSON/HTML 消费。
    单次结果，多消费方共用——summary/html/text 不再各自重扫 vault。"""
    targets = CHECK_ORDER if names == "all" else names
    results = []
    for name in targets:
        buf = io.StringIO()
        with contextlib.redirect_stdout(buf):
            count = CHECKS[name](root)
        results.append((name, int(count), buf.getvalue()))
    return results


def _aggregate(res):
    """由结构化结果算 aggregate：绿=全 0，红=任一 check 计数>阈值(恒 0)。"""
    total = sum(c for _, c, _ in res)
    failed = sum(1 for _, c, _ in res if c > 0)
    return {"total_issues": total, "checks_failed": failed,
            "status": "red" if failed > 0 else "green"}


def _exit_code(agg):
    return 0 if agg["status"] == "green" else 1


def _render_summary(root, names):
    res = run_checks(root, names)
    checks = {name: {"count": count, "threshold": 0, "pass": count <= 0}
              for name, count, _ in res}
    agg = _aggregate(res)
    return {
        "generated_at": datetime.now().astimezone().isoformat(timespec="seconds"),
        "vault": str(Path(root).resolve()),
        "checks": checks,
        "aggregate": agg,
        "exit_code": _exit_code(agg),
    }


def _render_html(summary, out_path):
    """自包含 HTML 健康页（aggregate + 每项计数），供浏览器查看趋势。"""
    checks_html = "".join(
        f'<div class="c {"ok" if v["pass"] else "bad"}">'
        f'<span>{name}</span><b>{v["count"]}</b></div>'
        for name, v in summary["checks"].items())
    agg = summary["aggregate"]
    page = (f'<!doctype html><html lang=zh><head><meta charset=utf-8>'
            f'<meta name=viewport content="width=device-width, initial-scale=1">'
            f"<title>知识库健康</title><style>"
            f"body{{font-family:system-ui;margin:2rem;background:#0d1117;color:#e6edf3}}"
            f".ag{{font-size:1.5rem;font-weight:700}}.ok{{color:#3fb950}}.bad{{color:#f85149}}"
            f".c{{display:flex;justify-content:space-between;padding:.3rem .5rem;border-bottom:1px solid #21262d}}"
            f"</style></head><body>"
            f"<h1>知识库健康巡检</h1>"
            f"<p>vault: {html.escape(summary['vault'], quote=True)} · 生成: {html.escape(summary['generated_at'], quote=True)}</p>"
            f"<p class='ag {agg['status']}'>状态: {agg['status'].upper()} · "
            f"问题合计: {agg['total_issues']} · 未通过项: {agg['checks_failed']}</p>"
            f"<div>{checks_html}</div></body></html>")
...
def main():
    ...
    parser.add_argument("--checks", default=None,
                        help="summary 专用：逗号分隔的 check 名（默认 all）")
    parser.add_argument("--html", metavar="OUT", default=None,
                        help="summary 专用：另生成自包含 HTML 健康页到 OUT")
    args = parser.parse_args()

    if args.root:
        root = Path(args.root).expanduser().resolve()
        if not root.exists():
            print(f"❌ vault 不存在: {root}", file=sys.stderr)
            sys.exit(3)
    else:
        root = ROOT_DEFAULT
    ...
    try:
        if args.check == "all":
            ...
            for name, count, text in run_checks(root, "all"):
                emit(f"===== 巡检: {name} =====", text)
                total += count
            summary = f"\n✅ 巡检完成 · 问题合计: {total}"
            print(summary)
            ...
            summary = _render_summary(root, args.checks.split(",") if args.checks else "all")
            if args.html:
                _render_html(summary, Path(args.html).expanduser())
            print(json.dumps(summary, ensure_ascii=False, indent=2))
            exit_code = summary["exit_code"]
        elif args.check in CHECKS:
            emit(f"===== 巡检: {args.check} =====\n", "")
            _, total, text = run_checks(root, [args.check])[0]
            emit(f"===== 巡检: {args.check} =====", text)
            ...
            print(f"用法: {sys.argv[0]} {'|'.join(CHECKS)} | all | summary（--checks/--html）（加 --log 写日志）")
            sys.exit(2)
    finally:
        if logfh:
            logfh.close()

...
# ── FR-8: 语义边界分块 ────────────────────────────────────────
def _find_semantic_boundaries(body: str) -> list:
    """识别语义边界位置：标题行、代码块结束、水平分割线。

    Args:
        body: 文档正文

    Returns:
        排序的偏移量列表（含 0 和 len(body)），表示可在这些位置切分。
    """
    boundaries = [0]
    lines = body.split("\n")
    offset = 0
    ...
    for line in lines:
        ...
        if stripped.startswith("```"):
            if in_code_block:
                # 代码块结束位置是下一行起始
                boundaries.append(offset + len(line) + 1)
                in_code_block = False
            else:
                in_code_block = True
        # 标题行（非代码块内）
        elif not in_code_block and re.match(r"^#{1,6}\s", stripped):
            boundaries.append(offset)
        # 水平分割线
        elif not in_code_block and re.match(r"^(-{3,}|\*{3,}|_{3,})$", stripped):
            boundaries.append(offset)
...
def _sliding_window_with_boundaries(body, chunk_size=2000, overlap=500):
    """边界感知分块：优先在语义边界处切分，段内超长时回退滑动窗口。
    ...
    Returns:
        分块文本列表
    """
    boundaries = _find_semantic_boundaries(body)
    # 如果只有首尾边界，说明无语义边界，回退滑动窗口
    if len(boundaries) <= 2:
        return _sliding_window(body, chunk_size, overlap)

    # 按语义边界分段
    segments = []
    for i in range(len(boundaries) - 1):
        seg = body[boundaries[i]:boundaries[i + 1]]
        if seg.strip():
            segments.append(seg)

    # 累积段到 chunk_size 产出一块
    chunks = []
    current = []
    current_len = 0
    ...
    for seg in segments:
        seg_len = len(seg)
        # 当前段本身超长 → 先产出已累积的，再对超长段回退滑动窗口
        if seg_len > chunk_size:
            if current:
                chunks.append("\n".join(current))
                current, current_len = [], 0
            # 对超长段回退滑动窗口
            sub_chunks = _sliding_window(seg, chunk_size, overlap)

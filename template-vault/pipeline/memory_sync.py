...
DUP_SIM = 0.92    # 与 KB 顶 sims≥此值 → 重复（增量，不新建）
NOVEL_SIM = 0.70  # 顶 sims<此值 → 新颖（增量候选）
...
def memory_files(root):
    out = []
    for p in clean.iter_notes(root):
        rel = str(p.relative_to(root))
        if rel.split(os.sep)[0] == "memory":
            fm, text, block, body = clean.load(p)
...
def grade(rel, fm, body, vocab, idf, rel2vec, meta_all, density_thr, min_chars, semantic):
    ...
    q1 = any(w in body for w in ["根因", "为什么", "原因", "因为", "所以", "导致"])
    q2 = len(matches) >= 2 and any(m[1] > 0.1 for m in matches[1:])
    q3 = any(w in body for w in ["长期", "持久", "预防", "反复", "每次", "总是", "原则", "机制"])
    soft_fail_reasons = []
    ...
    return {"rel": rel, "density": round(dens, 2), "best_rel": best_rel, "best_sim": round(best_sim, 3),
            ...
            "reasons": reasons, "pass": verdict, "domain": clean.infer_domain("", fm, "", body),
...
def build_note(root, rel_src, title, body, bucket, meta, dens, review_needed=False, soft_fail_reasons=None):
    ...
    domain = meta.get("domain", clean.infer_domain("", meta, title, body))
    created = clean.parse_date(src.stem, datetime.date.fromtimestamp(src.stat().st_mtime))
    today = datetime.date.today()
    fm2 = {
        "tags": clean.list_of(meta) or clean.infer_tags(bucket),
    ...
    text = "---\n" + "\n".join("%s: %s" % (k, clean.fmt_value(v)) for k, v in fm2.items()) \
        + "\n---\n\n" + body.lstrip("\n")

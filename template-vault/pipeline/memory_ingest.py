...
SYNC_SIM = 0.85     # 与 KB 已有 sims≥此 → 视为已同步(增量,不重复写)
REPLACE_SIM = 0.85  # 写入方式决策：sims≥此 → 替换（覆盖同标题 section）
MERGE_SIM = 0.70    # 0.70≤sims<0.85 → 合并（追加）；sims<0.70 → 新建 section
STATE_FILE = ".memory_ingest_state.json"


def _text_sim(a, b):
    """两段文本的 Jaccard 相似度（token 集合交并比），不依赖 vector index。"""
...
def load_state(root):
    ...
    try:
        ...
        return StateStore(root).load("memory_state.json", default)
    except ImportError:
        try:
...
def sync(root, agent_src, agent_name="openclaw"):
    ...
    for p in sorted(src.glob("*.md")):
        ...
        if kb_path.exists():
            ...
            sim = _text_sim(entry, existing)
            if sim >= REPLACE_SIM:
                # 替换：sims≥0.85 → 覆盖同标题 section（若存在）或整文件
                ...
                print(f"  ↻ 替换 {rel}（sims {sim:.2f}≥{REPLACE_SIM}）")
            else:
                # 合并/新建：sims<0.85 → 追加（0.70≤sims<0.85 合并 / sims<0.70 新建 section）
                kb_path.write_text(existing + "\n" + entry, encoding="utf-8")
                if sim >= MERGE_SIM:
                    print(f"  + 合并 {rel}（sims {sim:.2f}∈[{MERGE_SIM},{REPLACE_SIM})）")
                else:
                    print(f"  + 新建 section {rel}（sims {sim:.2f}<{MERGE_SIM}）")
...
def extract(root, agent_root, density_thr, min_chars, semantic, agent_name="openclaw"):
    ...
    for name in ("MEMORY.md", "USER.md"):
        ...
        for title, body, domain in rels:
            ...
            if rel is not None and sim >= DUP_SIM:

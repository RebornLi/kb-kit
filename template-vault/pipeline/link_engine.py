...
def suggestions(root: Union[str, Path], threshold: float) -> Tuple[List[str], Dict[str, Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
    notes, stem2rel, vecs = {}, {}, {}
    ...
    # 共享 token 为空 → 余弦必为 0 → 低于 threshold(默认 0.75) 可安全跳过。
    inv_index = build_inverted_index(vecs)

    # 桶内比对（同 domain）—— 使用 candidate_pairs 限制在桶内 rels 范围
    for domain, bucket_rels in buckets.items():
        for a, b in candidate_pairs(inv_index, bucket_rels):
    ...
    for i, da in enumerate(domains):
        for db in domains[i + 1:]:
            if da == "-" or db == "-":
                continue
            combined = buckets[da] + buckets[db]
            dom_of = {r: da for r in buckets[da]}
            dom_of.update({r: db for r in buckets[db]})
            for a, b in candidate_pairs(inv_index, combined):
                # 仅保留跨域对（同域对已在桶内比对中处理）
...
def build_report(rels: List[str], notes: Dict[str, Dict[str, Any]], recs: List[Dict[str, Any]],
                 cross: List[Dict[str, Any]], threshold: float) -> str:
    lines = [f"# 🔗 建议链接清单（连接引擎）",
             f"",
